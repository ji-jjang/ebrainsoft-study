...
const QuestionBoard = () => {
  ...
  const fetchQuestionBoardList = async (condition) => {
    const data = await getQuestionPostListApi(condition);

    setQuestionPosts(data.postList);
  ...
  useEffect(() => {
    ...
    fetchQuestionBoardList(updatedSearchCondition);
  }, []);

  const handleSearch = () => {
    fetchQuestionBoardList(searchCondition);
  ...
  return (
    <Container fluid>
      ...
      <Col className="text-end mt-3">
        <Button
          variant="primary"
          href={`/question-board/create`}
          onClick={(e) => {
            e.preventDefault();
            navigate(`/question-board/create${location.search}`);
          }}
        >
          글 등록
        </Button>
            ...
            <tbody>
              {questionPosts.map((post) => (
                <tr key={post.id} className="table-secondary">
                  ...
                  <td>
                    <a
                      href={`/question-board/post/${post.id}`}
                      onClick={(e) => {
                        e.preventDefault();
                        navigate(
                          `/question-board/post/${post.id}${location.search}`,
                        );
                      }}
                    >
                      {post.title}
